...
**Failure handling:** Treat the call as failed if no response is received within 12 seconds, or if the response is not valid JSON. On timeout or network failure, retry once after 2 seconds with a 5-second timeout. On a non-200 HTTP response, return `{ ok: false, reason: 'api_error' }` immediately — do not retry. If the retry also fails, render the rule-based fallback with the banner: "Using simplified plan — AI unavailable. Review steps before starting."
